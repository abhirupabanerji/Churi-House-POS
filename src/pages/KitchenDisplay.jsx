...
const urgencyColor = (mins) => {
  if (mins < 8) return "border-green-500/30";
  if (mins < 15) return "border-yellow-500";
  return "border-red-500 glow-orange-strong";
};
  ...
  useEffect(() => {
    const load = () => {
      base44.entities.Order.filter(
        ...
        .then(data => {
          const filtered = isAllBranches ? data : data.filter(o => {
            const ob = o.branch_name || o.branch || "";
            return ob.toLowerCase() === branch.toLowerCase();
          });
          setOrders(filtered);
        })
        .catch(err => console.error("Kitchen fetch error:", err));
    };
    load();
    ...
    return (
      <div className={`glass rounded-2xl p-4 border-l-4 ${urgencyColor(mins)} transition-all`}>
        ...
        <div className="flex gap-2">
          {order.status === "pending" && <Button size="sm" className="flex-1 h-9 bg-blue-600 hover:bg-blue-700 text-xs font-semibold" onClick={() => updateStatus(order.id, "preparing")}><ChefHat className="w-3.5 h-3.5 mr-1" /> Start Cooking</Button>}
          {order.status === "preparing" && <Button size="sm" className="flex-1 h-9 bg-green-600 hover:bg-green-700 text-xs font-semibold" onClick={() => updateStatus(order.id, "ready")}><CheckCircle className="w-3.5 h-3.5 mr-1" /> Mark Ready</Button>}
          {order.status === "ready" && <Button size="sm" className="flex-1 h-9 bg-purple-600 hover:bg-purple-700 text-xs font-semibold" onClick={() => updateStatus(order.id, "served")}>Served</Button>}
        </div>
      </div>
    );
  };
