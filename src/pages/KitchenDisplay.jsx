...
// ✅ FIX 1: Urgency color logic corrected (green = fresh, yellow = moderate, red = overdue)
...
export default function KitchenDisplay() {
  ...
  useEffect(() => {
    const load = () => {
      // ✅ FIX 2: Use status__in for multi-value filter (fixes orders not showing)
  ...
  const OrderCard = ({ order }) => {
    ...
    return (
      <div
        className={`glass rounded-2xl p-4 border-l-4 ${urgencyColor(mins)} transition-all`}
      >
        ...
        <div className="flex gap-2">
          ...
          {order.status === "preparing" && (
            <Button
              size="sm"
              className="flex-1 h-9 bg-green-600 hover:bg-green-700 text-xs font-semibold"
          ...
          {order.status === "ready" && (
            <Button
              size="sm"
              className="flex-1 h-9 bg-purple-600 hover:bg-purple-700 text-xs font-semibold"
