    ...
    <div className="space-y-2">
      ...
      {currentUrl ? (
        <div className="flex items-center gap-3 p-3 glass rounded-xl border border-white/10">
          <img src={currentUrl} alt="preview" className="h-10 w-10 object-contain rounded-lg bg-white/5" onError={(e) => { e.target.style.display = "none"; }} />
          <span className="text-xs text-muted-foreground flex-1 truncate">{currentUrl.startsWith("data:") ? "Uploaded image" : currentUrl.split("/").pop()}</span>
          <Button size="sm" variant="outline" className="h-7 px-2 bg-red-500/10 border-red-500/20 text-red-400 hover:bg-red-500/20" onClick={handleRemove}><X className="w-3 h-3" /></Button>
        </div>
      ) : (
...
export default function SettingsPage() {
  ...
  // Restore favicon from localStorage on every page load/refresh
  useEffect(() => {
  ...
  // Email fields are optional — only block if smtp_host is filled but dependents are missing
  const validate = () => {
    const next = {};
    if (tab === "general") {
  ...
  const syncToReceiptSettings = async (s) => {
    try {
      ...
      for (const rec of existing) {
        // Don't re-update the main settings record (settingsId)
  ...
  const save = async () => {
    ...
    try {
      ...
      // Persist favicon to browser tab
      ...
      // Sync shared fields to per-branch ReceiptSettings
  ...
  const uploadFile = async (field, file) => {
    ...
    try {
      ...
      // Apply favicon immediately to tab on upload
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      const backup = { version: "1.0", created_at: new Date().toISOString(), restaurant: settings.restaurant_name, data: { orders, inventory, users, branches, branchSettings, attendance } };
      const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `churihouse_backup_${new Date().toISOString().slice(0, 10)}.json`; a.click();
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      for (const list of [branchSettings, inventory, branches, attendance]) {
        if (list?.length) {
          const entity = list === branchSettings ? base44.entities.BranchSettings
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between glass rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div><Label>{isDark ? "Dark Mode" : "Light Mode"}</Label><p className="text-xs text-muted-foreground">Toggle between dark and light theme</p></div>
                  </div>
          ...
          {tab === "branding" && (
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="col-span-2"><Label>Receipt Header </Label><Input value={settings.receipt_header} onChange={e => update("receipt_header", e.target.value)} className="bg-white/5 border-white/10 mt-1" /></div>
                <div><Label>Receipt Footer </Label><Input value={settings.receipt_footer} onChange={e => update("receipt_footer", e.target.value)} className="bg-white/5 border-white/10 mt-1" /></div>
               </div>
              <SaveBtn />
            </>
          )}
          ...
          {tab === "email" && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><Label>SMTP Port</Label><Input value={settings.smtp_port} onChange={e => update("smtp_port", e.target.value)} className={`bg-white/5 border-white/10 mt-1 ${fieldError(errors, "smtp_port") ? "border-red-500" : ""}`} />{fieldError(errors, "smtp_port") && <p className="text-xs text-red-400 mt-1">{fieldError(errors, "smtp_port")}</p>}</div>
                <div><Label>From Email</Label><Input value={settings.from_email} onChange={e => update("from_email", e.target.value)} placeholder="noreply@churihouse.in" className={`bg-white/5 border-white/10 mt-1 ${fieldError(errors, "from_email") ? "border-red-500" : ""}`} />{fieldError(errors, "from_email") && <p className="text-xs text-red-400 mt-1">{fieldError(errors, "from_email")}</p>}</div>
                <div><Label>From Name</Label><Input value={settings.from_name} onChange={e => update("from_name", e.target.value)} className={`bg-white/5 border-white/10 mt-1 ${fieldError(errors, "from_name") ? "border-red-500" : ""}`} />{fieldError(errors, "from_name") && <p className="text-xs text-red-400 mt-1">{fieldError(errors, "from_name")}</p>}</div>
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="glass rounded-xl p-4 space-y-3 border border-yellow-500/20">
                  ...
                  <p className="text-xs text-muted-foreground">Upload a <strong>.json</strong> backup file to restore your data. Existing records will not be deleted — restored records will be added alongside them.</p>
                  <div className="space-y-2">
                  ...
                  <Button variant="outline" className="bg-yellow-500/10 border-yellow-500/20 text-yellow-400 hover:bg-yellow-500/20" onClick={handleRestore} disabled={!backupFile || restoring}>
