function makeLocalEntity(entityName) {
  ...
  return {
    ...
    async create(data) {
      ...
      const item = { id: crypto.randomUUID(), ...data, created_date: now, updated_date: now };
      rows.push(item);
      write(rows);
      return item;
    },
    async update(id, data) {
      const rows = read();
      const updated = rows.map(row => row.id === id ? { ...row, ...data, updated_date: new Date().toISOString() } : row);
      write(updated);
...
export const base44 = {
  ...
  auth: {
    async me() { return { email: "local@app.com", full_name: "Local User" }; },
    logout() {},
    redirectToLogin() {},
  },
  integrations: {
    Core: {
      async UploadFile() { throw new Error("Not available in local mode."); },
      async InvokeLLM() { throw new Error("Not available in local mode."); },
    },
  },
};